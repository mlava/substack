...
export class Shortcuts extends React.PureComponent {
    constructor() {
        super(...arguments);
        this.getShorcutClickHandler = (shortcut, index) => () => {
            const { onShortcutClick } = this.props;
            onShortcutClick(shortcut, index);
        };
        this.isShortcutInRange = (shortcutDateRange) => {
    ...
    render() {
        ...
        const shortcutElements = shortcuts.map((shortcut, index) => (React.createElement(MenuItem, { active: this.props.selectedShortcutIndex === index, className: Classes.POPOVER_DISMISS_OVERRIDE, disabled: !this.isShortcutInRange(shortcut.dateRange), key: index, onClick: this.getShorcutClickHandler(shortcut, index), text: shortcut.label })));
